...
function theme_scripts() {
    ...
    wp_localize_script('theme-script', 'screenReaderText', array(
        'expand' => __('expand child menu', 'phiabentronghoctu'),
        'collapse' => __('collapse child menu', 'phiabentronghoctu'),
    ));
}
...
function theme_widgets_init2() {
    register_sidebar(array(
        'name'          => __('Primary Widget Area', 'phiabentronghoctu'),
        'id'            => 'primary',
        'description'   => __('Add widgets here to appear in your sidebar on blog posts and archive pages.', 'phiabentronghoctu'),
...
function _theme_get_excerpt($args = array()) {
	...
	$more_tag                     = isset($args['more_tag'])                     ? $args['more_tag']                     : __('Read more', 'phiabentronghoctu');
...
function theme_get_category_list() {
	return str_replace(
		'<a',
		'<a class="u-textlink"',
		get_the_category_list(_x(', ', 'Used between list items, there is a space after the comma.', 'phiabentronghoctu'))
	);
}

function theme_get_tags_list() {
    return get_the_tag_list('', _x( ', ', 'Used between list items, there is a space after the comma.', 'phiabentronghoctu'));
}
...
function getProductButtonHtml ($button_html, $product, $clickTypeProductbutton = 'add-to-cart', $contentProductbutton = '') {
    ...
    $buttonText = sprintf(__('Add to cart', 'phiabentronghoctu'));
    if ($contentProductbutton !== '') {
        $buttonText =  sprintf(__($contentProductbutton, 'phiabentronghoctu'));
    }
...
function theme_404_content($args = '') {
    $args = wp_parse_args($args, array(
        'error_title' => __('Nothing here', 'phiabentronghoctu'),
        'error_message' => __('It looks like nothing was found at this location. Maybe try a search?', 'phiabentronghoctu')
    ));
    extract($args);
...
function blog_sorting() {
    $blog_sorting_options = array(
        'name_asc'  => __('Name, A to Z', 'phiabentronghoctu'),
        'name_desc' => __('Name, Z to A', 'phiabentronghoctu'),
        'date_asc'  => __('Date, old to new', 'phiabentronghoctu'),
        'date_desc' => __('Date, new to old', 'phiabentronghoctu'),
    );
